...
def main
  ...
  unsafe_flags = with_app_package("nitroSQLite" => {"threadSafe" => 0}) do
    flags_for(nil)
  end
  assert_threadsafe(unsafe_flags, "0")
  assert_optimization_flags(unsafe_flags)

  safe_flags = with_app_package("nitroSQLite" => {"threadSafe" => 0}) do
    flags_for("1")
  end
...
def assert_invalid_package_value_rejected
  with_app_package("nitroSQLite" => {"threadSafe" => 2}) do
