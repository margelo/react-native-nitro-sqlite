...
The connection helpers wrap database failures in `NitroSQLiteError`. This includes failed queries and queue errors such as trying to run a synchronous operation while an async operation owns the connection. Its `type` field identifies a recognized native exception category, such as `SqlExecutionError`, and is `undefined` for queue errors. It is not a numeric SQLite error code. See [types and errors](/docs/api/types-and-errors#nitrosqliteerror).
