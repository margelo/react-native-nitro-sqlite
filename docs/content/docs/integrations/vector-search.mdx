...
`CreateVectorTableOptions` requires `dimensions`. Its optional `type` is `'float'`, `'int8'`, or `'bit'`, with `'float'` as the default. `distanceMetric` accepts `'L2'`, `'cosine'`, or `'L1'`; sqlite-vec's default is L2. `column` defaults to `'embedding'`. `KnnSearchOptions` also lets you set `column`. For int8 and bit tables, construct vectors with sqlite-vec's `vec_int8()` and `vec_bit()` SQL functions as appropriate.
