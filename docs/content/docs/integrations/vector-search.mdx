...
- macOS: set `NITRO_SQLITE_VEC=1` when installing Pods from the app's `macos` directory. It uses the same Apple build flag as iOS.
...
Rebuild the native app after changing a flag. The companion package must be installed for the flag to resolve its source files. See the [iOS](/docs/configuration/ios#vector-search), [macOS](/docs/configuration/macos), and [Android](/docs/configuration/android#vector-search) configuration pages for details.
...
The helpers interpolate table and column identifiers into SQL. Only pass identifiers you control. They bind the query vector and `k` as parameters. The core [connection query methods](/api/react-native-nitro-sqlite/interfaces/NitroSQLiteConnection) can also run sqlite-vec SQL directly for features these small helpers do not wrap.
