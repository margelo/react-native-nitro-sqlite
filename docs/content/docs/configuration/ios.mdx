...
`threadSafe: false` compiles SQLite with `SQLITE_THREADSAFE=0` and removes its mutex code. Nitro SQLite rejects [independent connections](/docs/guides/multiple-connections) in that build. Separate database handles can still run concurrently, so an app using this setting must serialize SQLite calls across the process. `performanceMode: false` removes the pod's extra SQLite compile flags independently of thread safety. Those flags include `SQLITE_DQS=0`, `SQLITE_DEFAULT_MEMSTATUS=0`, `SQLITE_DEFAULT_WAL_SYNCHRONOUS=1`, and several `SQLITE_OMIT_*` options; this switch does not set a runtime SQLite pragma.
