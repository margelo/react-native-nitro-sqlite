...
Attach a second database to an open connection when a query needs tables from both files. The alias becomes a SQLite schema name. Use the same platform root and relative `location` convention as `open()`.
