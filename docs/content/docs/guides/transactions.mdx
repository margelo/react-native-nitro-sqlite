...
Use `db.transaction()` when several statements must succeed or fail together. It starts a SQLite transaction and passes a transaction object to an async callback. The callback's return value becomes the value of the transaction promise.
...
const transferId = await db.transaction(async (tx) => {
  ...
  const { results } = await tx.executeAsync<{ id: number }>(
    'INSERT INTO transfers (amount) VALUES (?) RETURNING id',
    [25],
  )
